...
export default function AppointmentParent() {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [tabdata, settabdata]=useState("upcoming");
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-[#f9f9fb]">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="px-4 md:px-6 py-4">
            {/* tabs */}
            <div>
                <AppointmentTabs onSelect={(val)=>{settabdata(val)}}/>
                <Tabsdatapages value={tabdata}/>
            </div>
        </div>
      </div>
    </div>
